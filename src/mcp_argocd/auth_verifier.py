...
mcp-argocd sits behind kube-rbac-proxy, which already gates on the
...
mcp-argocd learns who the call is for: a tank session's user, a
...
# Closed set of roles mcp-argocd accepts on inbound JWTs. Mirrors
...
# Cap signature/issuer/role check leeway on clock skew between mcp-argocd
...
CALLER: ContextVar[Caller | None] = ContextVar(
    "mcp_argocd_caller", default=None
)
